"""One-off script to generate final_train_crnn.ipynb — run: python notebooks/_gen_final_train_crnn_nb.py"""
import json
...
cells.append(
    md(
        r"""# `final_train_crnn.ipynb` — Fine-tune CRNN MobileNet v2.0 (PaddleOCR) cho bài toán Rec tiếng Nhật
...
> So với SVTR Tiny: model nhỏ hơn nhiều (MobileNetV3 small 0.5 + BiLSTM hidden 48) nên dùng batch **512** trên 1 GPU và LR **1.5e-3** (sqrt-scale từ `1e-3 @ batch 256` của config gốc PaddleOCR)."""
    )
)
...
Khai báo seed, đường dẫn workspace `/workspace`, vị trí dataset đã chuẩn bị (`/workspace/line_dataset`), thư mục output thí nghiệm `/workspace/exp_crnn_mobile_line` và `REC_IMAGE_SHAPE` cho CRNN."""
    )
)
...
OUTPUT_ROOT = WORK_DIR / "exp_crnn_mobile_line"
OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

CONFIG_PATH = OUTPUT_ROOT / "crnn_mobile_line.yml"
...
cells.append(
    md(
        """### Bước 5 — Tải pretrained `ch_ppocr_mobile_v2.0_rec_train`

Dùng CRNN MobileNet v2.0 tiếng Trung làm warm-start cho fine-tune tiếng Nhật. File tải về dạng `.tar`, giải nén vào `OUTPUT_ROOT/pretrained/`.

Cấu trúc tar (theo PaddleOCR dygraph v2.0): `best_accuracy.pdparams / .pdopt / .states` (+ có thể kèm `train.log`/yaml).

> Lưu ý: dict output của pretrained là tiếng Trung (~6625 chars) còn dict tiếng Nhật của ta khác → layer cuối `CTCHead.fc` sẽ không khớp shape và PaddleOCR sẽ **bỏ qua param không khớp** khi load pretrained (tự reinit). Phần Backbone + Neck (RNN) vẫn tận dụng được."""
    )
)
...
PRETRAINED_TAR = PRETRAINED_DIR / "ch_ppocr_mobile_v2.0_rec_train.tar"
PRETRAINED_ROOT = PRETRAINED_DIR / "ch_ppocr_mobile_v2.0_rec_train"
...
cells.append(
    md(
        """### Bước 6 — Sinh YAML config huấn luyện CRNN MobileNet
...
N_GPU = 1
BATCH_PER_CARD = int(globals().get("BATCH_PER_CARD_OVERRIDE", 512))
NUM_WORKERS = int(globals().get("NUM_WORKERS_OVERRIDE", 16))
...
def _load_pretrained_crnn_architecture(pretrained_root: Path) -> Dict:
    for yml_path in sorted(pretrained_root.glob("*.yml")):
        try:
            ...
            if isinstance(arch, dict) and arch.get("algorithm") == "CRNN":
                print("Use CRNN architecture from pretrained yaml:", yml_path.name)
                return arch
        except Exception as exc:
            print(f"Skip invalid yaml {yml_path.name}: {exc}")

    print("Fallback to built-in CRNN MobileNetV3 small 0.5 architecture defaults")
    return {
        "model_type": "rec",
        "algorithm": "CRNN",
        "Transform": None,
...
architecture_cfg = _load_pretrained_crnn_architecture(PRETRAINED_ROOT)
...
cells.append(
    md(
        """### Bước 8 — Inference trên tập **test**

Export checkpoint `best_accuracy` thành inference model, sau đó chạy `predict_rec.py` trên thư mục `test/` để lấy kết quả phục vụ tính EM/CER/NED. CRNN dùng `--rec_algorithm CRNN` (transform `RecResizeImg` mặc định, không cần patch gì thêm so với config train)."""
    )
)
...
cells.append(
    md(
        """### Bước 9 — Tính EM / CER / NED trên tập test
...
cells.append(
    md(
        """### Bước 10 — Resume training (đang để comment)
...
# RESUME_CONFIG_PATH = OUTPUT_ROOT / "crnn_mobile_line_resume.yml"
#
...
cells.append(
    md(
        """### Bước 11 — Đóng gói hai bản `.tar.gz` (deploy + resume)

Tạo **hai** file nén có timestamp trong `/workspace`:

- **`crnn_mobile_deploy_{timestamp}.tar.gz`** — mang đi **infer / đánh giá**: config YAML, `train.log`, metric test (`eval_test_em_cer_ned.json`), log/kết quả predict, thư mục `output/inference/`, `best_accuracy.pdparams`, `output/config.yml`, `dict_japanese.txt`. Không gồm `best_accuracy.pdopt`/`.states` (thường không cần cho inference-only).

- **`crnn_mobile_resume_{timestamp}.tar.gz`** — để **tiếp tục train** (Bước 10): thêm đủ `best_accuracy.*` và `latest.*` (`.pdparams`, `.pdopt`, `.states`), cùng config/log/metric/dict như trên.
...
DEPLOY_ARCHIVE = PACK_DIR / f"crnn_mobile_deploy_{timestamp}.tar.gz"
RESUME_ARCHIVE = PACK_DIR / f"crnn_mobile_resume_{timestamp}.tar.gz"
...
OUT = Path(__file__).resolve().parent / "final_train_crnn.ipynb"
